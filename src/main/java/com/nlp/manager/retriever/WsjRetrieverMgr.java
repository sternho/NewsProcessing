...
@Component
public class WsjRetrieverMgr {
	
	private static final Logger logger = LoggerFactory.getLogger(WsjRetrieverMgr.class);
	
	private static final SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
	
	private enum DATA_SOURCE {
		Market_Watch ("MAIN", "https://www.wsj.com/news/markets");
	...
	public void reportCurrentTime() {
		logger.info("WsjRetrieverMgr start process time: {}", dateFormat.format(new Date()));
		
		DATA_SOURCE[] links = DATA_SOURCE.values();
		for(DATA_SOURCE link : links) {
			try {
				HTMLParser html = new HTMLParser(link.getLink());
				Elements details = html.getDetailsByClass("a[class^=wsj-headline-link]");

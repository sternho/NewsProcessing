...
@Component
public class ReutersRetrieverMgr {
	
	private static final Logger logger = LoggerFactory.getLogger(ReutersRetrieverMgr.class);
	
	private static final SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
	
	private enum DATA_SOURCE {
		Market_Watch ("MAIN", "https://www.reuters.com/finance/markets");
	...
	public void reportCurrentTime() {
		logger.info("ReutersRetrieverMgr start process time: {}", dateFormat.format(new Date()));
		
		DATA_SOURCE[] links = DATA_SOURCE.values();
		for(DATA_SOURCE link : links) {
			try {
				HTMLParser html = new HTMLParser(link.getLink());
				Elements details = html.getDetailsByClass("h3[class^=story-title]");

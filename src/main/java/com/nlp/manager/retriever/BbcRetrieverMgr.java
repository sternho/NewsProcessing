...
@Component
public class BbcRetrieverMgr {
	
	private static final Logger logger = LoggerFactory.getLogger(BbcRetrieverMgr.class);
	
	private static final SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
	
	private enum DATA_SOURCE {
		Market_Watch ("MAIN", "https://www.bbc.com/news/business");
	...
	public void reportCurrentTime() {
		logger.info("BbcRetrieverMgr start process time: {}", dateFormat.format(new Date()));
		
		DATA_SOURCE[] links = DATA_SOURCE.values();
		for(DATA_SOURCE link : links) {
			try {
				HTMLParser html = new HTMLParser(link.getLink());
				Elements details = html.getDetailsByClass("span[class^=title-link__title-text]");
